...
Бинарник Xray хранится **на томе** (`/etc/x-ui/bin/`), поэтому обновление через кнопку в панели сохраняется между пересозданиями контейнера. Допустимо обновлять Xray из веб‑интерфейса: **Settings → Xray Core → Check for Updates → Update**.
...
**Важно о версиях Xray:** проект Xray‑core использует rolling‑release — стабильные «Latest»‑сборки выходят раз в несколько месяцев, а между ними все версии формально помечены как Pre‑release. В экосистеме 3x‑ui такие промежуточные версии считаются пригодными для использования (панель сама предлагает их к установке). Практический критерий: **если в течение 1–2 недель после выхода версии не появилось хотфикса — можно обновляться.** Перед обновлением обязательно сделать резервную копию тома (см. раздел «Резервное копирование» выше).

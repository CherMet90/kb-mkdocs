...
## Перенос MCP-профилей между устройствами

### Экспорт профиля
```bash
# 1. Узнать имя текущего профиля
docker mcp profile list

# 2. Создать папку для экспорта (если её нет)
mkdir -p ~/temp

# 3. Экспортировать профиль (в примере "default")
docker mcp profile export default ~/temp/default-profile.json
```

### Импорт на другом устройстве
```bash
# 1. На целевом устройстве создать папку и поместить туда файл профиля
#    (флешка, облако, scp)
mkdir -p ~/temp

# 2. Импортировать профиль
docker mcp profile import ~/temp/default-profile.json
```

**Примечание:**  
- **Важно:** версии Docker Desktop на обоих устройствах должны совпадать.  
- **Важно:** если папка `~/temp` не существует — создать её перед экспортом/импортом.  
- Профиль содержит все активные MCP-серверы, их настройки и точки подключения.  
- Можно использовать и путь в Windows (`C:\temp\...`), логика та же.
